...
class TestSmoothers:
    ...
    def test_impute_with_savgol(self):
        ...
        assert np.allclose(imputed_X, np.hstack([np.arange(11), np.arange(10)]))
        ...
        X = np.hstack([np.arange(5), [np.nan], np.arange(20), [np.nan], np.arange(5)])
        imputed_X = smoother.impute_with_savgol(X, window_length, poly_fit_degree=2)
        assert np.allclose(
            imputed_X, np.hstack([np.arange(6), np.arange(21), np.arange(5)]),
        )
